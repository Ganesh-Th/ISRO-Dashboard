...
def create_connection():
    ...
    try:
        connection = mysql.connector.connect(
            host='localhost',
            user='root',
            password='Ganesh#2003',  # Replace with your MySQL password
...
def insert_data(defect_no, satellite, component_name, component_id, defects_detected, date):
    ...
    cursor = connection.cursor()
    query = """
    INSERT INTO defect_data (defect_no, satellite, component_name, component_id, defects_detected, date)
    VALUES (%s, %s, %s, %s, %s, %s)
    """
    try:
...
def fetch_latest_entry():
    ...
    query = "SELECT * FROM defect_data ORDER BY id DESC LIMIT 1"
...
options = ["Xray Evaluation Results", "Xray Image Defect Detection – Screen 1", "Defect Information"]
choice = st.radio("Choose an option", options)

if choice == "Xray Evaluation Results":
    st.header("Xray Evaluation Results")
...
elif choice == "Xray Image Defect Detection – Screen 1":
    st.title("Xray Image Defect Detection – Screen 1")
